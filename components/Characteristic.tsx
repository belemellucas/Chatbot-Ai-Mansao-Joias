...
function Characteristic({
    ...
    return (
        <li key={characteristic.id} className="relative flex p-10 bg-white border rounded-md">
        <div>{characteristic.content}</div>
       <div>
        <OctagonX className="w-6 h-6 text-white fill-red-500 absolute top-2 right-2 cursor-pointer hover:opacity-50" 
        onClick={() => {
